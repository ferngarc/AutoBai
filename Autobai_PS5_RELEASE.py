...
def form(console, supplier):
    root = tkinter.Tk()
    root.title("Fernando's PlayStation Bot")
    ...
    CreditExpireMonthEntered = tkinter.Entry(
        root, width=5, textvariable=myCreditExpireMonth
    )
    ...
    CreditExpireYearEntered = tkinter.Entry(
        root, width=5, textvariable=myCreditExpireYear
    )
...
if __name__ == "__main__":
    ...
    root.title("Fernando's PlayStation Bot")
    root.geometry("500x500")
    greeting = tkinter.Label(root, text="Welcome to Fernando's PlayStation 5 Bot")
    greeting.grid(column=25, row=0)
    about1 = tkinter.Label(
        root,
        text="This program will run scripts and automates buying a PlayStation 5 or Xbox Series X/S. Please select the console you desire, and you'll be greeted with a form to fill out to automate shipping and payment",
        wraplength=450,
    )
